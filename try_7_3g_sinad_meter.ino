...
//
//  To be checked
//  1.  Sine wave DDS OSCILLO view pending
//  2.  WAVE triggered
//
#include <Arduino.h>
#include <Wire.h>
...
void drawWAVE_n() {                     // Wave after notched
  display.drawLine(0, 28, 127, 26, SSD1306_WHITE);  // 32->26
  drawWAVEMEMORI();
  for (int i = 0; i < SAMPLE_SIZE - 1; i++) {
    int x1 = map(i, 0, SAMPLE_SIZE - 1, 0, SCREEN_WIDTH - 1);
  ...
  if (millis() - lastSampleTime >= 200) {
    ...
    for (int i = 0; i < SAMPLE_SIZE; i++) {
      ...
      if(testwaveEnabled == true){
        ...
        inputBuffer[i] = sin(2.0 * PI * 1000.0 * i / 10000.0);  // 1kHz, Fs=10kHz
